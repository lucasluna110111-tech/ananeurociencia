...
const PRODUCTS = {
  'guia-neurociencia': {
    id: 'guia-neurociencia-001',
    title: 'Guía Completa de Neurociencia Aplicada, Neuroplasticidad y Aprendizaje',
    r2Key: 'guia-neurociencia.pdf',
    downloadName: 'Guia-Neurociencia-Aplicada.pdf'
  },

  'guia-memoria': {
    id: 'guia-memoria-001',
    title: 'Guía de Memoria',
    r2Key: 'Guia_de_Memoria_Anna_Irene-v4 (1).pdf',
    downloadName: 'Guia-de-Memoria.pdf'
  }
};
...
function paymentMatches(payment, token, product) {
  return !!payment &&
    ...
    payment.metadata?.product === product;
}

async function getSale(env, token) {
  return env.DB
    .prepare('SELECT * FROM sales WHERE token = ?')
    .bind(token)
    .first();
}
...
async function createCheckout(request, env) {
  ...
  if (!env.DB) {
    return json({
      error: 'Falta vincular la base D1 con el nombre DB.'
    }, 500);
  }
  ...
  const productKey = body.product || 'guia-neurociencia';
  ...
  await env.DB.prepare(`
    INSERT INTO sales
    (token, product, amount, currency, created_at, approved, used)
    VALUES (?, ?, ?, ?, ?, 0, 0)
  `)
    .bind(
      token,
  ...
  try {
    ...
    await env.DB
      .prepare('DELETE FROM sales WHERE token = ?')
      .bind(token)
      .run();
  ...
  if (!response.ok || !data.init_point) {
    ...
    return json({
      error: 'Mercado Pago no pudo crear el checkout.',
      detail: data
    }, 502);
  }
...
async function paymentStatus(request, env) {
  ...
  const product = PRODUCTS[sale.product];

  if (
    !product ||
    Number(sale.amount) !== PRICE ||
  ...
  if (Number(sale.used) === 1) {
    return json({
      status: 'used',
      message: 'Este enlace de descarga ya fue utilizado.'
    });
  }
  ...
  if (!payment) {
    return json({
      status: 'pending',
      message: 'Todavía no se pudo consultar el pago.'
    });
  }
  ...
  if (
    ...
    return json({
      error: 'El pago no coincide con esta compra.'
    }, 403);
  }
  ...
  return json({
    ...
    downloadUrl:
      `/api/download?token=${encodeURIComponent(token)}&payment_id=${encodeURIComponent(payment.id)}`
  });
}
...
async function download(request, env) {
  ...
  if (!env.GUIDES) {
    return new Response(
      'Falta vincular el bucket R2 con el nombre GUIDES.',
      { status: 500 }
    );
  }
  ...
  if (
    Number(sale.approved) !== 1 ||
    String(sale.payment_id) !== String(paymentId)
  ) {
    return new Response(
  ...
  const object = await env.GUIDES.get(product.r2Key);
